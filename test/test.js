...
test("buildIframeSrc builds a valid URL which includes only applicable params passed to it", (t) => {
    ...
    const options = { id: 'uk1vy9x3yq5b515xvfhv2', width: 640, height: 360, colour: '00ffff' };

    const src = buildIframeSrc(options);

    const expectedSrc = 'https://embed.vidbeo.com/uk1vy9x3yq5b515xvfhv2?colour=00ffff'
...
test('postMessage sends a message for a method which does not need a value', (t) => {
    ...
    t.true(spy.calledWith({ type: 'method', name: 'exampleMethod', value: null }, 'example.com'));
});
...
test('postMessage sends a message with value for a method which does need a value', (t) => {
    ...
    t.true(spy.calledWith({ type: 'method', name: 'setColour', value: '00ffff' }, 'example.com'));
});
...
test('a player can have callbacks attached using on to listen for events', (t) => {
    ...
    t.throws(() => player.on(), { instanceOf: Error, message: 'An event is required' });
    t.throws(() => player.on('madeupeevent'), { instanceOf: Error, message: 'That event is currently not supported' });
    t.throws(() => player.on('timeupdate'), { instanceOf: Error, message: 'A callback is required and it must be a function' });
    t.throws(() => player.on('timeupdate', 'notafunction'), { instanceOf: Error, message: 'A callback is required and it must be a function' });
});
